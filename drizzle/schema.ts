...
/**
 * Session summaries generated by AI after each coaching conversation.
 * Stores structured insights for future growth review and personalization.
 * Generated automatically when a session ends (user closes or 10min inactivity).
 * Not visible to users — internal data only.
 */
export const sessionSummaries = pgTable("session_summaries", {
  ...
  /** Links to users.id */
  userId: integer("userId").notNull(),
  /** Guardian name (e.g., "Axel", "Andy", "Anya", "Alma") */
  guardian: varchar("guardian", { length: 64 }).notNull(),
  /** Core topic of the conversation (one sentence) */
  topic: text("topic"),
  /** The most important insight the user reached (one sentence) */
  keyInsight: text("keyInsight"),
  /** User's emotional state at end of session (e.g., calm, activated, overwhelmed, hopeful) */
  emotionalState: varchar("emotionalState", { length: 64 }),
  /** Action the user committed to, if any */
  actionCommitted: text("actionCommitted"),
  /** Total number of dialogue rounds in the session */
  sessionRounds: integer("sessionRounds").default(0).notNull(),
  /** 2-3 sentence overview of the conversation */
  summary: text("summary"),
  /** Timestamp of the conversation date */
