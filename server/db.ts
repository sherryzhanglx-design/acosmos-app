...
export async function upsertUser(user: InsertUser): Promise<{ isNewUser: boolean }> {
  ...
  try {
    ...
    } else if (user.email === 'sherryzhang@lifemaster.coach' || user.openId === ENV.ownerGoogleId || (ENV.ownerOpenId && user.openId === ENV.ownerOpenId)) {
      values.role = 'admin';
...
export async function seedDefaultRoles(): Promise<void> {
  ...
  const defaultRoles: InsertCoachingRole[] = [
    ...
    { slug: "family", name: "Amos", description: "Guardian of Family — nurturing family bonds and intergenerational wisdom", icon: "Family", color: "#a78bfa", avatar: "https://files.manuscdn.com/user_upload_by_module/session_file/310519663273647099/eOVjFlKTtQnFuGnK.png", sortOrder: 7 },
